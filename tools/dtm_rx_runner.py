...
class DtmRxRunner:
    ...
    def _build_ui(self) -> None:
        ...
        # Wi-Fi pre-test toggle: when enabled, the DUT runs the Wi-Fi FTM
        # factory script (blocking) BEFORE each BT TX/RX measurement. Turn
        # it OFF to A/B-test whether the Wi-Fi teardown is what kills the
        # BT RX (rx_count==0).
        self.wifi_pretest_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(cfg, text="Wi-Fi pre-test",
                        variable=self.wifi_pretest_var,
    ...
    def _dut_wifi_test(self) -> None:
        """Run the Wi-Fi FTM factory test on the DUT and BLOCK until it
        finishes. Called BEFORE the BT TX/RX test so the Wi-Fi run never
        overlaps the BT RX measurement window.

        Skipped in standalone mode (no SSH)."""
        if self.is_standalone():
            self.write("[STANDALONE] Skip Wi-Fi test (no SSH).")
            return
        if not self.wifi_pretest_var.get():
            self.write("[DUT] Wi-Fi pre-test disabled (toggle off) - skipping.")
            return
        script_dir = dut_control.DEFAULT_SCRIPT_DIR
        if self.is_manual():
            script_dir, _eh, _ep, _tx, _off = self._manual_params()
        self.write(f"[DUT] {dut_control.DEFAULT_WIFI_SCRIPT}  "
                   f"(Wi-Fi pre-test, dir={script_dir})")
        self._dut_call(dut_control.run_wifi_test,
                       session=self.ssh,
    ...
    def _dut_test_async(self) -> None:
        ...
        if self.is_manual():
            ...
            self._dut_call_async(dut_control.run_bt_tx_test,
                                 ...
                                 script_dir=script_dir)
            return
        # DUT-LINK: drive the controller directly so the TX channel always
        # matches the dongle RX channel and the TX stays on for the whole RX
        # window. BLOCK until the LE Transmitter Test is actually started
        # (controller-init time no longer matters); TX is stopped later by
        # _dut_tx_off() (bt_test_off.sh).
        ch = self.ch_var.get()
        length = self.len_var.get()
        self.write(f"[DUT] BT DTM TX start (ch={ch}, len={length}) "
                   f"via persistent SSH")
        self._dut_call(dut_control.run_bt_tx_dtm,
                       session=self.ssh, channel=ch, length=length)
    ...
    def on_start(self) -> None:
        ...
        # 0) Wi-Fi FTM factory test first (blocking). Runs before BT TX so
        #    it never overlaps the BT RX measurement window.
        self._dut_wifi_test()
        ...
        try:
            for i in range(1, iterations + 1):
                ...
                self.write(f"\n--- AUTO {i}/{iterations}  Test #{self.test_index} "
                           f"(ch={ch}, len={length}) ---")
                # Run the Wi-Fi FTM factory test FIRST and let it finish.
                # It is blocking, so the BT RX window below never overlaps it.
                self._stage_async(f"[{i}/{iterations}] Wi-Fi test",
